...
def show_venue(venue_id):
  ...
  past_shows = Venue.query.filter(Venue.start_time < datetime.now() and Venue.id == venue_id).all()

  num_past_shows = Venue.query.filter(Venue.start_time < datetime.now() and Venue.id == venue_id).count()

  upcoming_shows = Venue.query.filter(Venue.start_time > datetime.now() and Venue.id == venue_id).all()

  num_upcoming_shows = Venue.query.filter(Venue.start_time > datetime.now() and Venue.id == venue_id).count()
...
def search_artists():
  ...
  for artist in search_result:
    ...
    response["data"]["num_upcoming_shows"] = Show.query.filter(Show.start_time > datetime.now() and Show.artist_id == artist.id).count()
